...
def update_diffraction_panel(
    ...
    # Actualizar anotación con offset dinámico
    if gd_c > -3.0:
        y_offset = -0.5
        va = "top"
    else:
        y_offset = 0.5
        va = "bottom"
    artists.text_annotation.set_position((v_c + 0.05, gd_c + y_offset))
    artists.text_annotation.set_verticalalignment(va)
    ...
    # Asegurar que el límite superior de Y no choque con el título
    y_min, _ = ax.get_ylim()
    ax.set_ylim(y_min, 6.0)
